...
void filterVectorErr(std::vector<T> &v, UnaryPredicate p)
{
    v.erase(v.begin(), unstable_remove_if(v.begin(), v.end(), p));
}
...
template<bool negCommand>
void quickStartGuyComputeMove(MyRobot &me, Simulator &sim)
{
    constexpr F INV_Z = negCommand ? 1 : -1;
    ...
    if (!me.touch)
    {
        me.action.target_velocity = P3(0, -MAX_ENTITY_SPEED, 0);
    ...
    bool jump = length(me.body.pos - sim.ball.pos) < BALL_RADIUS + ROBOT_MAX_RADIUS && me.body.pos.z * INV_Z < sim.ball.pos.z * INV_Z;
    ...
    bool is_attacker = sim.robots.size() == 2;
    for (MyRobot &robot : sim.robots)
    {
        if (robot.side == me.side && robot.id != me.id)
        {
            if (robot.body.pos.z * INV_Z <= me.body.pos.z * INV_Z)
            {
                is_attacker = true;
            }
        }
    ...
    if (is_attacker)
    {
        ...
        {
            F t = i * 0.1;
            P3 ball_pos = sim.ball.pos + sim.ball.vel * t;
            ...
            if (ball_pos.z * INV_Z > me.body.pos.z * INV_Z
                && std::abs(ball_pos.x) < (sim.arena->width / 2.0)
                && std::abs(ball_pos.z) < (sim.arena->depth / 2.0))
            {
                // Посчитаем, с какой скоростью робот должен бежать,
                // Чтобы прийти туда же, где будет мяч, в то же самое время
                P delta_pos = P(ball_pos.x, ball_pos.z) - P(me.body.pos.x, me.body.pos.z);
                F need_speed = delta_pos.len() / t;
                    ...
                    && need_speed < ROBOT_MAX_GROUND_SPEED)
                {
                    ...
                    P target_velocity = delta_pos.norm() * need_speed;

                    me.action.target_velocity = P3(target_velocity.x, 0, target_velocity.z);
                    me.action.jump_speed = jump ? ROBOT_MAX_JUMP_SPEED : 0;
    ...
    P target_pos = P(0.0, -(sim.arena->depth / 2.0) + sim.arena->bottom_radius) * INV_Z;
    // Причем, если мяч движется в сторону наших ворот
    if (sim.ball.vel.z * INV_Z < -EPS)
    {
        // Найдем время и место, в котором мяч пересечет линию ворот
        F t = (target_pos.z - sim.ball.pos.z) / sim.ball.vel.z;
        F x = sim.ball.pos.x + sim.ball.vel.x * t;
        // Если это место - внутри ворот
        if (std::abs(x) < (sim.arena->goal_width / 2.0))
        {
    ...
    P target_velocity = P(
            target_pos.x - me.body.pos.x,
            target_pos.z - me.body.pos.z
...
void QuickStartGuy::compute(const Simulator &_sim)
{
    this->sim = _sim;
    ...
    for (MyRobot &me : sim.robots)
    {
        ...
        quickStartGuyComputeMove<true>(me, sim);
    }
}
...
namespace stratV24 {
    ...
    EngagePoint computeEngagePoint(const Simulator &sim, const MyRobot &me, int count, int enEngageT)
    {
        ...
        for (size_t i = 0; i < count; )
        {
            if (i > 0)
            {
                if (!result.predictedBallIterator.computeTick())
                    break;
            }
    ...
    void MyStrat::computeJump(MyRobot &me)
    {
        ...
        if (st.targetT < DEPTH_T && me.touch)
        {
            ...
            //for (int k = 0; k < 1; ++k)
            {
                ...
                if (jumpTime == 0)
                {
                    ...
                    if (enemyBallDist < 1 || score > 100 || score > currentScore + 500 || sim.ball.pos.z < -5)
                    {
    ...
    F computeTrajScore(const Simulator &sim, int meId, const Trajectory2d &traj, int enEngageT)
    {
        ...
        int trajLen = traj.vel.size() + traj.flyT;
    ...
    F computeFlyScore(const Simulator &sim, int meId, int enEngageT, P3 targetVel, int nitroTicks)
    {
        ...
        for (int t = 0; t < 30; ++t)
        {
            ...
            if (t < nitroTicks)
            {
                tsim.robots[ind].action.use_nitro = true;
            }
            else
            {
                tsim.robots[ind].action.use_nitro = false;
            }
    ...
    KickParams findKickParams(const Simulator &sim, const MyRobot &me, const Ball &b, const P&vel, int ct)
    {
        KickParams result;
        ...
        for (size_t nitroTp = 0; nitroTp < 3; ++nitroTp)
        {
            ...
            for (int k = -5; k <= 5; k += 1)
            {
                for (int t = 0; t < JUMP_CONSTANTS.jumpAttrs[nitroTp].size() && ct >= t; ++t)
                {
                    ...
                    if (R2 > dh2)
                    {
                        ...
                        F score = ballScoreFast(tsim, sim.robots) - nitroTp * NITRO_POINTS;
    ...
    void MyStrat::updateRobotState()
    {
        ...
        for (MyRobot &me : sim.robots)
        {
            if (me.side == Side::POS)
            {
                engagePoints.insert(std::make_pair(me.id, computeEngagePoint(sim, me, PRED_COUNT, -1)));
                ...
                F l = me.body.pos.xz().dist(sim.ball.pos.xz());
                if (len > l)
                {
                    closestEnId = me.id;
                    len = l;
        ...
        for (MyRobot &me : sim.robots)
        {
            if (me.side == Side::NEG)
            {
                engagePoints.insert(std::make_pair(me.id, computeEngagePoint(sim, me, PRED_COUNT, enEngageT)));
            ...
            if (engagePoint.found && engagePoint.targetT < 60 && me.side == Side::NEG && me.touch)
            {
                for (int ct = engagePoint.targetT; ct < engagePoint.targetT + 6; ++ct)
                {
                    ...
                    for (int iter = 0; iter < 2; ++iter)
                    {
                        ...
                        KickParams kickParams = findKickParams(sim, me, b, spd, ct);
                    ...
                    if (!engagePoint.predictedBallIterator.computeTick())
                        break;
                }
            }
        ...
        for (MyRobot &me : sim.robots)
        {
            ...
            /*if (bestTrajScoreId != -1 && me.id != bestTrajScoreId)
            {
                st.traj.found = false;
            }*/
            ...
            if (!st.traj.found)
            {
                ...
                auto compScore = [this, &st](const P &p, const MyRobot &me){
                    F s = 0;
                    Robot2d robot2d = Robot2d(me);
                    F expF = 1.0;
                    for (int t = 0; t <= 15; ++t)
    ...
    void MyStrat::compute(const Simulator &_sim)
    {
        ...
        for (MyRobot &me : sim.robots)
        {
            ...
            if (st.nitroTicks > 0 && (!me.touch || me.action.jump_speed > 0))
            {
                if (!me.touch)
                {
                    ...
                    auto upd = [&](F f, F s1, F s2){
                        ...
                        F newScore = computeFlyScore(sim, me.id, enEngageT, newTargetVel, st.nitroTicks);
                        if (newScore > score)
                        {
...
namespace stratV25 {
    ...
    EngagePoint computeEngagePoint(const Simulator &sim, const MyRobot &me, int count, int enEngageT, std::map<int, RobotState> &robotState)
    {
        EngagePoint result = EngagePoint(sim, me);
    ...
    void MyStrat::updateRobotState()
    {
        ...
        F currentBallScore = ballScoreFast(sim, sim.robots);
        ...
        for (MyRobot &me : sim.robots)
        {
            if (me.side == Side::POS)
            {
                engagePoints.insert(std::make_pair(me.id, computeEngagePoint(sim, me, PRED_COUNT, -1, robotState)));
                EngagePoint &engagePoint = engagePoints.find(me.id)->second;
                if (engagePoint.found && enEngageT > engagePoint.targetT)
                {
                    enEngageT = engagePoint.targetT;
        ...
        for (MyRobot &me : sim.robots)
        {
            if (me.side == Side::NEG)
            {
                engagePoints.insert(std::make_pair(me.id, computeEngagePoint(sim, me, PRED_COUNT, enEngageT, robotState)));
    ...
    void MyStrat::computeHighDanger()
    {
        ...
        if (currentBallScore < -600)
        {
            ...
            for (const MyRobot &me : sim.robots)
            {
                ...
                {
                    RobotState &st = robotState[me.id];
                    if (st.traj.found || st.targetT < 80)
                    {
                        highDanger = false;
                        break;
                    }
